...
class LRADataset(Dataset):
    ...
    def create_inst(self, inst):
        output = {}
        output["input_ids_0"] = torch.tensor(inst["input_ids_0"], dtype = torch.float)
        output["mask_0"] = (output["input_ids_0"] != 0).float()
        if "input_ids_1" in inst:
            output["input_ids_1"] = torch.tensor(inst["input_ids_1"], dtype = torch.float)
    ...
    def get_labels(self) :
        return np.array([inst["label"] for inst in self.examples])

    def get_x(self) :
        return np.array([inst["input_ids_0"] for inst in self.examples])
