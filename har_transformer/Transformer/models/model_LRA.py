...
#            token_out, attn_loss_lst, attn_lst = self.model(input_ids_0, mask_0, mat_lst, is_attn)
            token_out = self.model(input_ids_0, mask_0, mat_lst, is_attn)
            ...
            outputs["pred"] = seq_scores
#            outputs["attn_loss"] = attn_loss_lst
...
#        self.vocab_size = config["vocab_size"]
    ...
    def forward(self, input_ids_0, input_ids_1, mask_0, mask_1, label, mat_lst=[], is_attn=False):
        ...
        with torch.cuda.amp.autocast(enabled = self.enable_amp):
            ...
            token_out_0 = self.model(input_ids_0, mask_0, mat_lst, is_attn)
            token_out_1 = self.model(input_ids_1, mask_1, mat_lst, is_attn)
            
#            token_out_0, attn_loss_lst_0, attn_lst_0 = self.model(input_ids_0, mask_0, mat_lst, is_attn)
#            token_out_1, attn_loss_lst_1, attn_lst_1 = self.model(input_ids_1, mask_1, mat_lst, is_attn)
            ...
            outputs["loss"] = seq_loss
            outputs["accu"] = seq_accu
#            outputs["attn_loss_0"] = attn_loss_lst_0
#            outputs["attn_loss_1"] = attn_loss_lst_1
            """if self.inference:
