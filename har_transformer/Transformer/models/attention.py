...
#        self.pattern = nn.Parameter(torch.rand(self.num_blocks, self.head_dim))
#        self.avg_pool = torch.nn.AvgPool2d(self.block_size)
...
#                    self.attn = CUDAAttention(config, attn_cpp, attn_handle)
        ...
        self.attn = SoftmaxAttention(config, inference)
